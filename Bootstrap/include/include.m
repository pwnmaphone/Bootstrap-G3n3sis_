...
//  Bootstrap
//
//  Created by Chris Coding on 12/30/23.
//
...
void _offsets_init(void) {
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"16.0")) {
        printf("[i] offsets selected for iOS 16\n");
        ...
        off_vnode_v_iocount = 0x64; // - done
        off_vnode_v_usecount = 0x60; // - done
        off_vnode_v_flag = 0x54; // - done
        off_vnode_v_name = 0xb8;
        off_vnode_v_mount = 0xd8;
        off_vnode_v_data = 0xe0;
        off_vnode_v_kusecount = 0x5c; // - done
        off_vnode_v_references = 0x5b;
        off_vnode_v_lflag = 0x58; // - done
        off_vnode_v_owner = 0x68;
        off_vnode_v_parent = 0xc0;
        off_vnode_v_label = 0xe8;
        off_vnode_v_cred = 0x98;
        off_vnode_v_writecount = 0xb0; // - done
        off_vnode_v_type = 0x70;
        off_vnode_vu_ubcinfo = 0x78; // - done
        ...
        if (SYSTEM_VERSION_LESS_THAN_OR_EQUAL_TO(@"16.3.1") && SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"16.0")) {
            printf("[i] extra offsets selected for iOS 16.0 - 16.3.1\n");
        ...
        } else if(SYSTEM_VERSION_EQUAL_TO(@"16.4")) { //Starting with iOS 16.4~
            printf("[i] extra offsets selected for iOS 16.4\n");
        ...
        } else if (SYSTEM_VERSION_LESS_THAN(@"16.0")) { // the other offsets may be wrong, but we're only using the vnode offsets & namecache
            printf("[i] offsets selected for iOS 15\n"); // which should be the same on iOS 15 & 16
            // iPhone Xr iOS 15.0.1 offsets
